...
class Menta_PHPUnit_Listener_VerboseResultPrinter extends PHPUnit_TextUI_ResultPrinter {
	...
	public function addFailure(PHPUnit_Framework_Test $test, PHPUnit_Framework_AssertionFailedError $e, $time) {
		$message = sprintf("%s  FAILURE: %s (Duration: %s sec)\n", str_repeat('  ', $this->level), str_replace("\n", " ", $e->getMessage()), $this->getDuration());
	...
	public function addSkippedTest(PHPUnit_Framework_Test $test, Exception $e, $time) {
		// $this->startTest($test);
		$message = sprintf("%s  SKIPPED: %s (Duration: %s sec)\n", str_repeat('  ', $this->level), str_replace("\n", " ", $e->getMessage()), $this->getDuration());
	...
	public function endTest(PHPUnit_Framework_Test $test, $time) {
		if (!$this->lastTestFailed) {
			$message = sprintf("%s  SUCCESS. (Duration: %s sec)\n", str_repeat('  ', $this->level), $this->getDuration());
			$this->write($message);
		}
		$this->level--;
